...
/*   Updated: 2024/04/12 21:13:50 by rihoy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lstcmd	*create_lstcmd(t_infopars *lst, t_shell *bash)
{
	...
	while (curr)
	{
		...
		define_last(curr, tmp_cmd, bash); // recommence pour definir here.
		if (tmp_cmd->in_file_name)
			printf_error("cmd name: %s\n", tmp_cmd->in_file_name);
		if (tmp_cmd->last_infile != -1)
			printf_error("cmd fd: %d\n", tmp_cmd->last_infile);
		free(tmp_cmd->in_file_name); // ne pas oublie de free plus haut
